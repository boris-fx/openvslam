...
openvslam_bfx::config_settings * settings_from_yaml(YAML::Node yaml_node)
{
    openvslam_bfx::config_settings * settings = nullptr;

    // Essential camera settings
    const auto camera_node = yaml_node["Camera"];

    openvslam::camera::model_type_t camera_model;
    const auto model_str = camera_node["model"].as<std::string>();
    if (model_str == "perspective")
        camera_model = openvslam::camera::model_type_t::Perspective;
    else if (model_str == "fisheye")
        camera_model = openvslam::camera::model_type_t::Fisheye;
    else if (model_str == "equirectangular")
        camera_model = openvslam::camera::model_type_t::Equirectangular;
    else if (model_str == "radial_division")
        camera_model = openvslam::camera::model_type_t::RadialDivision;
    else
    ...
    if (setup_type_str == "monocular")
        camera_setup = openvslam::camera::setup_type_t::Monocular;
    else if (setup_type_str == "stereo")
        camera_setup = openvslam::camera::setup_type_t::Stereo;
    else if (setup_type_str == "RGBD")
        camera_setup = openvslam::camera::setup_type_t::RGBD;
    else
        throw std::runtime_error("Invalid setup type: " + setup_type_str);
    
    openvslam::camera::color_order_t colour_order = openvslam::camera::color_order_t::Gray;
    if (camera_node["color_order"])
    {
        const auto colour_order_str = camera_node["color_order"].as<std::string>();
        if (colour_order_str == "Gray")
            colour_order = openvslam::camera::color_order_t::Gray;
        else if (colour_order_str == "RGB" || colour_order_str == "RGBA")
            colour_order = openvslam::camera::color_order_t::RGB;
        else if (colour_order_str == "BGR" || colour_order_str == "BGRA")
            colour_order = openvslam::camera::color_order_t::BGR;
        else
    ...
    switch (camera_model)
    {
        case openvslam::camera::model_type_t::Perspective:
        {
    ...
    if ( orb_node.size() ) {
        ...
        settings->orb_scale_factor_ = orb_node["scale_factor"].as<float>(1.2);
    }
    ...
    if ( keyframe_node.size() ) {
        settings->max_keyframe_interval_ = keyframe_node["max_interval"].as<double>(1.0);
    ...
    if ( solve_node.size() ) {
        settings->solve_use_fixed_seed_= solve_node["use_fixed_seed"].as<bool>(true);
        settings->num_ransac_iterations_= solve_node["num_ransac_iterations"].as<unsigned int>(100);
        settings->min_num_triangulated_ = solve_node["num_min_triangulated_pts"].as<unsigned int>(50);
    ...
    if ( loop_node.size() ) {
        ...
        settings->loop_detector_use_fixed_seed_= loop_node["use_fixed_seed"].as<bool>(true);
    ...
    if ( reloc_node.size() ) {
        settings->relocalizer_use_fixed_seed_ = reloc_node["use_fixed_seed"].as<bool>(true);
    ...
    // BoW
    const auto bow_node = yaml_optional_ref(yaml_node, "BowDatabase");
    if ( bow_node.size() ) {
        settings->reject_by_graph_distance_ = bow_node["reject_by_graph_distance"].as<bool>(false);
        settings->loop_min_distance_on_graph_ = bow_node["loop_min_distance_on_graph"].as<int>(30);
    }
...
void mono_tracking(const std::shared_ptr<openvslam::config>& cfg,
    ...
    pangolin_viewer::viewer viewer(
        openvslam::util::yaml_optional_ref(yaml_node, "PangolinViewer"), &SLAM, SLAM.get_frame_publisher(), SLAM.get_map_publisher());
#elif USE_SOCKET_PUBLISHER
    socket_publisher::publisher publisher(
        openvslam::util::yaml_optional_ref(yaml_node, "SocketPublisher"), &SLAM, SLAM.get_frame_publisher(), SLAM.get_map_publisher());
#endif
...
int main(int argc, char* argv[]) {
    ...
    std::shared_ptr<openvslam::config> cfg = std::make_shared<openvslam::config>(*settings);
