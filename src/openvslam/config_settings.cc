...
{
    if (camera_type != openvslam::camera::model_type_t::Perspective)
...
config_settings::config_settings(openvslam::camera::model_type_t camera_type,
    ...
    cols_(cols), rows_(rows), fps_(fps)
{
    if (camera_type != openvslam::camera::model_type_t::Fisheye)
        throw std::runtime_error("Incorrect camera settings for type");
...
config_settings::config_settings(openvslam::camera::model_type_t camera_type,
    ...
    cols_(cols), rows_(rows), fps_(fps)
{
    if (camera_type != openvslam::camera::model_type_t::RadialDivision)
        throw std::runtime_error("Incorrect camera settings for type");
...
config_settings::config_settings(openvslam::camera::model_type_t camera_type,
    ...
    cols_(cols), rows_(rows), fps_(fps)
{
    if (camera_type != openvslam::camera::model_type_t::Equirectangular)
        throw std::runtime_error("Incorrect camera settings for type");
}
...
std::ostream& operator<<(std::ostream& os, const config_settings& settings) {
    ...
    os << "Camera\n";
    os << "\tModel: " <<
        openvslam::camera::model_type_to_string[static_cast<unsigned>(settings.camera_type_)]
        << std::endl;
    os << "\tSetup: " <<
        openvslam::camera::setup_type_to_string[static_cast<unsigned>(settings.camera_setup_)]
        << std::endl;
    os << "\tColor order: " <<
        openvslam::camera::color_order_to_string[static_cast<unsigned>(settings.colour_order_)]
        << std::endl;
    ...
    os << "\tORB scale factor: " << settings.orb_scale_factor_ << std::endl;
    ...
    os << "KeyframeInserter\n";
    os << "\tMax keyframe interval: " << settings.max_keyframe_interval_ << std::endl;
    ...
    os << "Initializer\n";
    os << "\tSolve use fixed seed: " << settings.solve_use_fixed_seed_ << std::endl;
    os << "\tNum ransac iterations: " << settings.num_ransac_iterations_ << std::endl;
    os << "\tMin num triangulated: " << settings.min_num_triangulated_ << std::endl;
    ...
    os << "\tLoop detector use fixed seed: " << settings.loop_detector_use_fixed_seed_ << std::endl;
    ...
    os << "Relocalizer\n";
    os << "\tRelocalizer use fixed seed: " << settings.relocalizer_use_fixed_seed_ << std::endl;
    ...
    os << "\tLoop min distance on graph: " << settings.loop_min_distance_on_graph_ << std::endl;
